...
#include <APPLICATIONS/session/server/wimax/VideoTelephony.hpp>

using namespace applications::session::server::wimax;

STATIC_FACTORY_REGISTER_WITH_CREATOR(applications::session::server::wimax::VideoTelephony,
				     applications::session::Session,
				     "server.WiMAXVideoTelephony", wns::PyConfigViewCreator);
  ...
  cnCounter(0)
{
  ...
  packetFrom = "server.WiMAXVideoTelephony";
}
...
VideoTelephony::onData(const wns::osi::PDUPtr& _pdu)
{
  ...
  if((receivedPacketNumber == 1) && (state != sessionended))
    {
      MESSAGE_SINGLE(NORMAL, logger, "APPL: Call from client received!");
