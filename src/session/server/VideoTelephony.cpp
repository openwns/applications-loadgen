...
#include <APPLICATIONS/session/server/VideoTelephony.hpp>
#include <WNS/distribution/Norm.hpp>

using namespace applications::session::server;

STATIC_FACTORY_REGISTER_WITH_CREATOR(applications::session::server::VideoTelephony,
				     applications::session::Session,
				     "server.VideoTelephony", wns::PyConfigViewCreator);
  ...
  newGOP(true)
{
  ...
  MESSAGE_BEGIN(NORMAL, logger, m, "APPL: Starting new session with video parameters: ");
  ...
  packetFrom = "server.VideoTelephony";
}
